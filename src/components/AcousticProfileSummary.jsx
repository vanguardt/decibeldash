...
export default function AcousticProfileSummary({ wpmHistory }) {
  ...
  return (
    <motion.div
      ...
      className="w-full mt-8 bg-card border border-border rounded-xl p-4"
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 bg-background border border-border rounded-lg p-3 text-center">
        ...
        <div className="flex-1 bg-background border border-border rounded-lg p-3 text-center">
          <p className="text-[9px] uppercase tracking-widest text-muted-foreground">Signature</p>
          {sigStyle ? (
            <span className={`inline-block mt-1 text-xs px-2.5 py-0.5 rounded-full border ${sigStyle.className}`}>
              {sigStyle.label}
            </span>
          ) : (
          ...
          <div className="bg-background border border-border rounded-lg p-2.5">
            ...
            <div className="flex flex-wrap gap-1">
              {loudKeys.slice(0, 3).map((k) => (
                <span key={k.key} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-orange-500/10 text-orange-400">
          ...
          <div className="bg-background border border-border rounded-lg p-2.5">
            ...
            <div className="flex flex-wrap gap-1">
              {quietKeys.slice(0, 3).map((k) => (
                <span key={k.key} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400">
                  {k.label} {Math.round(k.delta)}
                </span>
              ))}
      ...
      {/* Score rings */}
      <div className="flex items-start justify-around pt-3 border-t border-border">
        <ScoreRing
